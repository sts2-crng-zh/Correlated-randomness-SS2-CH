const data = [
  [[12, 1176], [13, 7745], [14, 7763], [15, 7696], [16, 7775], [17, 2212], [27, 6223], [28, 7315], [29, 7260], [30, 7337], [31, 6627], [32, 5189], [33, 6246], [34, 9914], [35, 10106], [36, 4757], [37, 4873], [38, 1134]],
...
function buildGrid(div) {
  ...
  for (let i = 0; i < 11; ++i) {
    ...
    for (let j = 0; j < w; ++j) {
      ...
      el.style.outline = "1px solid var(--border, #888)";
      el.style.width = "33px";
  ...
  for (let i = 0; i < 8; ++i) {
    ...
    for (let j = 0; j < w; ++j) {
      ...
      box.className = "divination-box";
      box.style.gridArea = `${1 + i} / ${5 - w / 2 + j} / span 4 / span 4`;
      div.append(box);

      const lbl = document.createElement("div");
      lbl.className = "divination-label";
...
window.addEventListener("load", () => {
  const div = document.getElementById("divination-grid");
  const gold = document.getElementById("divgold");
  if (!div || !gold) return;

  const cells = buildGrid(div);
  const update = () => go(cells, Math.min(8, Math.max(0, (parseInt(gold.value, 10) || 7) - 7)));

  update();
  gold.addEventListener("change", update);
  gold.addEventListener("input", update);
});
